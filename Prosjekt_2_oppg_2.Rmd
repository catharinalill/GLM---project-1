...
simulation_matrix<- matrix(0,  1000, 16)

for (i in 1:1000){

  predict_goals = round(predict(mod, long, type="response"))
  predict_long <- long 
  
  predict_long$goals <-predict_goals

  #t<-team_rank(predict_long,480)
  #rbind(simulation_matrix_1,t)
  t<-team_rank(predict_long,480)
  for (j in 1:16){
     simulation_matrix[i ,j ] <- t[j]
  }
}
