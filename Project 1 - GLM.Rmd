...
myglm <- function(formula, dataset) {
  ...
  while (epsilon > 10^(-4)) {
    ...
    F_inv  <- solve(F)                                  # the Fisher information matrix to be able                                                         # to invert the matrix
    print(i)
    i <- i+1
...
#deviance(glm(cbind(y,n-y) ~ ldose, binomial, data=beetle))



```
Our model gives a deviance of $11.23$. When one looks at the definition of the deviance one can see that it is in fact the difference between logarithm of to probablities. When testing with some numerical values one gets $log(0.7)-log(0.6) = 0.15$ even when this is multiplied by $8*2$ which is the number of groups we get $2.46$ which is a lot smaller than our deviance. In other words if the cantidate model is very good one would expect a very low deviance. When testing $8*2*(log(0.9)-log(0.4))$ one gets $12.97$. This implies taht with a deviance of $11.23$ one can have quite a big difference between the candidate model and the saturated model implieng that the model is not a very good one. 
