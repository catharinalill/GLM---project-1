...
```{r, include=TRUE}
  ...
  p_val
```
From the output we see that $\hat{\phi}$ is larger than 1. We know the deviance is chisquared distributed and we have $G-p = 110$ degrees of freedom. When doing a p-test of the deviance we get that this is significant, thus we have overdispersion. Reasons for overdispersion can be unobserved heterogeneity and correlation between independent binary observations of the response variable. In this case, the data could be correlated e.g because of similar weather conditions. It would be easier for the expeditions to reach the summit if the weather is nice. The different expeditions within a group could also be correlated because of each other. If it is several expeditions attempting to reach the same mountain simultaneous it may affect each expeditions probability to reach the summit. It could be more difficult to reach the summit, but the expeditions could also help each other to reach the summit. 
...
#F-test for covariate height
anova(mod2, mod_height, test = "F")
#F-test for covariate prominence
anova(mod2, mod_prominence, test = "F")
...
#F-test
drop1(mod2, test = "F")  #We have a quasi-model so we need F-test, instead of LRT-test
...
p <- 3
df <- n-p   #n?????
p_val <- pf(wald, df,df,df, lower.tail = FALSE)
p_val
...
In the F-test we have nested models $H_0$ and $H_1$. Under $H_0$, we then have thes test statistic
$$
  \frac{(D_0-D_1)/(p_1-p_0)}{D_1/(n-p_1)} \sim F_{p_1-p_0,n-p_1}.
$$
where $D_i$ is the deviance for the model under $H_i$, and $p_i$ is the number of estimated regression coefficients for the model under $H_i$. So, here $p_1$ = 3, and $p_0$ = 2. 
 
By use of F-test we see that when testing $H_0:$ model with only height as covariate is better than $H_1:$ the full model, we get a p-value that is greater than a significance level of e.g. 0.05. Thus we cannot reject $H_0$ and we cannot say that the model with only height as a covariate is any worse than the full model. This indicates that the covariate prominence does not tell us that much. 

From the second F-test when testing $H_0:$ model with only prominence as covariate is better than $H_1:$ the full model, we get a p-value that is smaller than a significance level of 0.05. Thus we reject the null hypothesis that the model with only prominence is better than the full model. That is, height is an important covariate in this model and is thus significant. 
...
$$
where $i = \{0, height, prominence\}$. The result of these test are shown in the output of the summary of the full model. For the covariate height the p-value is very small, so we reject the null hypothesis. Thus, the height is a significant covariate. For the prominence the p-value is right above 0.05. So, here one have to decide which significance level to use. If one uses 0.05, we cannot reject $H_0$ so prominence is not significant. But, if one uses a significance level that is a bit higher, we can reject the null hypothesis and then prominence is also a significant covariate. Either way this tells us that height is the most significant covariate in the model. 

If we compare the wald-tests against the F-tests we see that the results are quite similar. There are some small differences in the test statistics tests due to using asymptotic distributions, but the conclusion of the tests are the same.
...
#t-test vs 
#LRT, tilpasse to modeller, H0 uten og H1 med kovariaten og sammenligner likelihood
#Wald, Z = beta/SE(beta), std normal eller med overdispersjon, T = beta/SEbeta estimator 

#LRT uten overdispersjon, 2(l(beta0)-l(beta1))
#LRT med overdisp, F = (D0-D1)/(p1-po)/D1/(n-p1)
#anova(mod0,mod1, test = "F")
#ER DET TO FRIHETSGRADER HER, eller må vi ta hensyn til phi også som en parameter?? skal ikke tas hensyn til her
```
each term in the model is significant. By comparing with the summary from the fitted model we see that both covariates are a bit less significant in the summary output. 
...
We then get a test-statistic
$$
  T = 
$$

From the result we can see that the p-value is much less than any reasonable significance level, say e.g 0.05. Hence, we reject the null hypothesis that the coefficient estimates are zero. Thus the coefficients are significant. 
