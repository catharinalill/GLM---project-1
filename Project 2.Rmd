...
Choose log as our linkfunktion since this is the natural choise for a poisson distribution. 

We assume that the number of goels is poison distributed and independens between the observations. This is not that realistic since to teams playing against each other probably will affect each others chance of making a goal. 

We also asume that the only facotors who influece the number of goals are the denfence team, the attack team ans weather they play on the home field or not. This is a very simple model and one could imagine that factors such as weather, illness, ect can make a difference. 
...
```
As one can see the correlation is high. This supports the claim.

We now want to check if there is overdispurtion in the data. Do do so we calculate the p-value. 
...
This p-value is not significant and one can not conclude with ovredispertion. Still possible sources of overdispertion could be correlated matches.
...
Now we want to simulate 1000 matches based on our model. 
...
One could assume that if each team is equally good at defence and attack and that these parameters therefor should be equal. 
This is a linear hypothesis because it can be written as $C\beta = d$. When the $\beta_attack$ =$-\beta_defence$ this simply gives a $C$ matrix with two onec on every row one at the place for the teams attack and one at the place for the temas defence. 

Under the construction of C is shown as well as a wald test of this hypotheses.
...
As one can see this gives a low p-value and we reject the nullhypotheses with significanse $0.0051$
