...
```
By use of likelihood ratio test we see that each term in the model is significant. From this test we can also see that both covariates is significant.(STÅR DET IKKE BARE DET SAMME HER TO GANGER?) By comparing with the summary from the fitted model we see that both covariates are less significant, but that the covariate prominence has a relatively large change is significance relative to the covariate height. (Skjønner dere hva som menes her?)
...
$$
Hence, the odds changes by $\exp(\beta_2)$. So, if $\beta_2 >0$ then $P(y_i=1)/P(y_i=0)$ increases and if $\beta_2 < 0$ then $P(y_i=1)/P(y_i=0)$ decreases. Also, if $\beta_2 = 0$ then $P(y_i=1)/P(y_i=0)$ remain unchanged. Hence, the effect of the covariates on the odds in exponentially multiplicative. 
...
Det er positiv korrelasjon her sant???
...
#HVORDAN KOMMER VI FREM TIL 352 her??
...
Her må vi forklare litt mer hva vi gjør, men tror jeg trenger en gjennomgang av hva som skjer først :) :) 
...
```
As one can see this gives a low p-value and we reject the nullhypothesis for any reasonable significance level. Hence, the simplification of the model to assume that the attack and defence strengths of each team are equal in absolute value but with opposite sign is not reasonable to do. ?? Er det riktig å si? 
Hva med å skrive det over i stedet? "with significance $0.0051$"
