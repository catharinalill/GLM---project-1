...
We now fit a glm modelling where the probability that an attempt at reaching a particular summit depends on its height and prominence. 
```{r, include=TRUE}
...
If good teams play well both in attack and defence one would expect these parameters to be negatively correlated. This is because if a team is good in attack $\beta_{attack}$ will be large and positive, and if it is good in defence then $\beta_{defence}$ will be large and negative. Thus, we get a negative correlation. The same applies if a team is bad in both attack and defence. 

```{r, include=TRUE}
cor.test(mod$coefficients[2:16], mod$coefficients[17:31]) #hva skjer med dummi parameteren? hva tenker du på her? Vi sjekker bare correlasjonen mellom 15 lag så vet ikke hva som skjer med lag nr 16 siden det er dummien, meen tror ikke det har noe å si. Spurte Jarle og vi skal ikke sjekke opp mot home :) Sweet!! :D 
```
As one can see from the output above, the correlation is high. This supports the claim that attack and defence are negatively correlated. This could be a possible source for overdispurtion which we will look at next. DET STEMMER SANT? KOMMENTERE NOE MER HER? 

##c)
We now want to check if there is overdispersion in the data. To do so we estimate the overdispersion parameter $\phi$, which is given in problem $2b)$. We know this parameter is chisquared distributed and from the summary we can see we have $352$ degrees of freedom, so we use this to calculate the p-value.   
```{r, include=TRUE}
deviance <- deviance(mod)
...
We observe a tendensy of overdispurtion but when calculatign the p-value we see that this p-value is not significant and one can not conclude with overdispersion. Still possible sources of overdispersion could be correlated matches and correlation between the defence and attack parameter. 

POSSIBLE SOURCES OF UNDERDISPERSION? TRENGER VI DET? ER DET NOEN ANNET EN KILDENE TIL OVERDISPURTION?
Sources?? overdispersion i 1c?? 
...
d  #Hva med å sortere denne?? DET KAN VI GODT, SÅ LENGE DET IKKE BLIR GJORT INNI FUNKSJONEN. SENERE ER VI AVHENGIG AV SAMME REKKEFLGE PÅ LAGENE NÅR DET BRUKES I SIMULERINGEN. 
...
av_result_sorted <-av_result[order(av_result$Expected),] #order the dataframe by the expected rank NYDELIG!!!! 
av_result_sorted 
```
From the output above we see that Rosenborg has the highest expected rank. 
KOMMENTERE NOE HER? ROSENBORG HAR HAR HØYEST EXPECTED RANK. TROR DET ER FINT MED SETNINGEN OVER :) 

##f)

One could assume that if each team is equally good at defence and attack these parameters should be equal in absolute value but of opposite sign. This is a linear hypothesis because it can be written as $C\beta = d$. When the $\beta_{attack}$ =$-\beta_{defence}$ this simply gives a $C$ matrix with two ones in every row, one at the place for the teams attack and one at the place for the teams defence. 
...
alpha = 0.05 #FIKK FEILMELDING ALPHA NOT DEFINED SÅ LA TIL DENNE
d <- rep(0,15)
...
qchisq(1-alpha,df=15)  #bruk denne istedenfor pchisq? også forkaste fordi denne er større enn wald? 

#HER SKJØNNER JEG IKKE HELT HVA SOM SKJER MEN DET ER SIKKERT RIKTIG! 
```
