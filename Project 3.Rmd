...
l_p <- function(theta){
  ...
  print(V(theta))
  print("DETERMINANT:")
  print(determinant(V(theta)))
  l_p = -1/2*(log(determinant(V(theta)))+(t(y-X%*%beta_est)%*%solve(V(theta))%*%(y-X%*%beta_est)))
  print("LP:")
  print(l_p)
  return(l_p)
}
...
mylmm <- function(y, group,REML = FALSE){ #i grupper #j obs i hver gruppe
  ...
  if (REML == FALSE) {
    ...
    obj = optim(par = theta,fn = l_r(theta),control = list(fnscale=-1))
  }
