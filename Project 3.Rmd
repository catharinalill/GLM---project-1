...
The effect on the log-odds is additive, meaning if we have a one unit change in the $period_i$ the change in the log-odds will be $\beta_i$. Hence, if we have a one unit change, the effect on the odds is multiplicative by a factor $exp(\beta_i)$. Thus, the ratio between odds for period 2 relative to period 1 for a given herd is 0.379 and the ratio for all herds is 0.401. This tells us that for a given herd there is a larger change in the odds-ratio/odds????, than for the whole population. 
