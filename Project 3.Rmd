...
By testing different number of quadrature points we see that five or six seems to be enough for the estimates of $\beta$ to stabilize.  ??? stabiliserer på 10??
...
summary(mod_laplace)
```
By comparing the summaries of the two fitted model, we can see only slightly changes in some of the estimates. For example the estimate of $\tau$ is a bit lower for the model fitted by using 5 ???? quadratures points compared to the refitted model. 
