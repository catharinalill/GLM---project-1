...
l_p <- function(theta,X){
  beta_est=beta(theta,X)
...
mylmm <- function(y, group,REML = FALSE){ #i grupper #j obs i hver gruppe
  ...
  X <- matrix(1,m*n)
  theta <- rep(0,2) #sigma, tau
  
  if (REML == FALSE) {
    optim(c(0,0),theta,l_p(theta,X),control = list(fnscale=-1),data = data)
  }
  else {
    optim(c(0,0),theta,l_r(theta,X),control = list(fnscale=-1),data = data)
  }
