...
This model gives conditionally independent $Y_{ij}$s but they are not marginally indepentent.

```{r}
library(lme4)

#BRUKER QUADTRATURE POINTS MÅ TESTE UT FOR HVILKEN nAGQ DEN STABILISERER SEG
mod <- glmer(cbind(incidence, size-incidence)~period +(1|herd), family=binomial(link = "logit"), nAGQ = 1, data = cbpp)
...
#laplac er det samme som å sette nAGQ =1 det er også default setting i glmer vi kommer til å se litt endringer i estimater og tau
summary(mod)
```
```{r}
help(glmmTMB)
