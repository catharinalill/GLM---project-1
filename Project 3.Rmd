...
summary(mod_laplace)
```
By comparing the summaries of the two fitted models, we can see only slightly changes in some of the estimates. For example the estimate of $\tau_0$ is a bit lower for the model fitted by using 5 ???? quadratures points compared to the refitted model. Also the estimates for $\beta$ are slightly changed. 
