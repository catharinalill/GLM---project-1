...
By testing different number of quadrature points we see that from around 6 quadrature points we get a numerical low enough error. This error will be much lower than the statistical error and is therefor good enough. 
...
summary(mod_laplace)
```
By comparing the summaries of the two fitted models, we can see only slightly changes in some of the estimates. For example the estimate of $\tau_0^2$ is a bit larger for the model fitted by using 10 quadratures points compared to the refitted model. Also the estimates for $\beta$ are slightly changed. 
...
summary(mod_reml)
```
As we can see from the summary output above, the estimate of $\tau_0^2$ is a bit larger than for the previous models. This is due to that fitting the model with REML gives a less biased variance. Hence, $\tau_0^2$ should be closer to its actual value. When fitting with maximum likelihood the variance get more biased and is underfitted. Hence, as expect the estimate for $\tau_0^2$ get a bit larger when using restricted maximum likelihood instead of maximum likelihood. 
...
To further investigate if this estimate wiht REML is preferable over the maximum likelihood estimate we want to investigate if the bias actually is smaller. To do so one can simulate many realisations from a known model with known parameters to create many new datasets. Then one could fit all of these datasets again and retreve new estimates for $\tau$. Then one could compute the average of these $\tau$s and compare this to the original estimate. One would expect that the difference would be smaller when using REML versus ML.  
...
modfixed <- glmmTMB(cbind(incidence, size-incidence)~1 +(1|herd), family=binomial(link = "logit"), data = cbpp, REML=FALSE)
...
gammas = rnorm(1000, 0, tau) #skal denne være roten av??? Jarle skrev N(0, T_0^2)
#gammas
#perios 1
pi_logit_en = beta[1]  + gammas

#period 2
pi_logit_to= beta[1] + beta[2] + gammas

#get the numbers for new insidences
pi_en =  exp(pi_log_en)/(1 + exp(pi_log_en))
pi_to =  exp(pi_log_to)/(1 + exp(pi_log_to))

Y_en = dbinom(1, 20, pi_en)
Y_to = dbinom(1, 20, pi_to)
#mangler å trekke y fra bin men 20, pi
#find the correlation
cor(Y_en, Y_to)

```
By simulating a thousand realisations of gammas and then computing the corresponding responses for a herd with fixed size 20 we compute the intraclass correlation between period one and two. We get a very high correlation ($0.9908$). This means that the results from period one and two resemble each other a lot. This suggests that the random variable is of great importance. 
